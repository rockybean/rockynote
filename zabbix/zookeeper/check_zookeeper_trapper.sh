...
ZK_HOST=$(awk -F= '/^ListenIP/ { print $2 } ' /etc/zabbix/zabbix_agentd.conf)

trap "rm -f $TMPFILE; rm -f $TMPFILE2 ; exit" SIGHUP SIGINT SIGTERM EXIT

echo "mntr" | curl -s telnet://${ZK_HOST}:2181 > $TMPFILE

RUNNING_OK=$(echo "ruok" | curl -s telnet://${ZK_HOST}:2181)
